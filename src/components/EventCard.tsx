...
export function EventCard({ event, onLike, liked = false, compact = false, featured = false, className }: EventCardProps) {
  ...
  const catColor = primaryCat?.slug ? CATEGORY_COLORS[primaryCat.slug] : '#C2582A'
  const catBg = getCategoryBg(primaryCat?.slug)
  const imageUrl = event.image_url || `https://picsum.photos/seed/${event.id}/800/500`
      ...
      <>
        ...
        >
          ...
          <img
            ...
            onError={(e) => {
              (e.target as HTMLImageElement).src = `https://picsum.photos/seed/${event.id}x/800/500`
            }}
          />
          ...
          <button
            ...
            className={cn(
              ...
              'backdrop-blur-sm',
              isLiked ? 'bg-[#C2582A] text-white' : 'bg-black/40 text-white/70 hover:text-white'
            )}
    ...
    <>
      ...
      >
        ...
        <div className={cn('relative overflow-hidden', compact ? 'h-40' : 'h-52')}
          ...
          {/* Category icon in corner if no image or as overlay */}
          {!event.image_url && IconComponent && (
            <div className="absolute inset-0 flex items-center justify-center">
              <IconComponent size={48} color={catColor} />
            </div>
          )}
          ...
          {event.is_free === 1 && (
            <div className="absolute top-3 left-3">
              <span className="bg-[#1C1C1E] text-white text-xs font-semibold tracking-wide px-2 py-1">
                FREE
              </span>
            </div>
          )}
          ...
          <button
            onClick={handleLike}
            className={cn(
              'absolute top-3 right-3 w-8 h-8 flex items-center justify-center transition-all duration-150',
              'backdrop-blur-sm',
              isLiked ? 'bg-[#C2582A] text-white' : 'bg-black/30 text-white/70 hover:text-white opacity-0 group-hover:opacity-100'
            )}
            aria-label="Like event"
          >
          ...
          <button
            ...
            className="absolute bottom-3 right-3 w-7 h-7 flex items-center justify-center bg-black/30 text-white/70 hover:text-white backdrop-blur-sm transition-all duration-150 opacity-0 group-hover:opacity-100"
        ...
        <div className="pt-3 pb-4">
          ...
          {/* Title */}
          <h3 className="font-display font-bold text-[#1C1C1E] leading-tight mb-2 transition-colors group-hover:text-[#C2582A]"
          ...
          {event.is_free !== 1 && priceLabel && (
            <div className="mt-1.5 text-xs font-semibold text-[#C2582A]">
              {priceLabel}
            </div>
          )}
        </div>
